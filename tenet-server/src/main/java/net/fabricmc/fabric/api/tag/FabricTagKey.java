...
public interface FabricTagKey {
	...
	default String getTranslationKey() {
		...
		ResourceLocation registryResourceLocation = tagKey.registry().registry();
		ResourceLocation tagResourceLocation = tagKey.location();

		if (!registryResourceLocation.getNamespace().equals(ResourceLocation.DEFAULT_NAMESPACE)) {
			stringBuilder.append(registryResourceLocation.getNamespace())
					.append(".");
		}

		stringBuilder.append(registryResourceLocation.getPath().replace("/", "."))
				.append(".")
