...
public class IdMapperTracker<V, OV> implements RegistryEntryAddedCallback<V>, RegistryIdRemapCallback<V> {
	...
	public static <V, OV> void register(Registry<V> registry, String name, IdMapper<OV> mappers) {
		IdMapperTracker<V, OV> updater = new IdMapperTracker<>(name, mappers);
	...
	public void onRemap(RemapState<V> state) {
		((RemovableIdMapper<OV>) mappers).fabric_remapIds(state.getRawIdChangeMap());
	}
}
